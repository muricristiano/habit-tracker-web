...
export function HabitDay({ amount = 0, defaultCompleted = 0, date}: HabitDayProps) {
    ...
    return(
        <Popover.Root>
            <Popover.Trigger 
                className={clsx("w-10 h-10 border-2 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 focus:ring-offset-zinc-900", {
                    ...
                    'bg-violet-400 border-violet-100': completedPercentage >= 100,
                    'border-white border-4' : isCurrentDay
                })}
            />

            <Popover.Portal>
                <Popover.Content className='min-w-[320px] p-6 rounded-2xl bg-zinc-900 flex flex-col focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 focus:ring-offset-zinc-900'>
